...
export default function CheckoutComponent() {
  ...
  const { items, totalPrice, restaurantId } = useCart();
  const [formData, setFormData] = useState({
    address: "",
    phone: user?.phone,
    notes: "",
  });
  const { data: menu } = useMenu(String(restaurantId) || "");

  const enrichedItems = items.map((cartItem) => {
    const menuItem = menu?.find(
      (m:any) => String(m.id) === String(cartItem.menuItemId),
    );
    return {
      ...cartItem,
  ...
  const hasUnavailable = enrichedItems.some(
      ...
      <Container maxWidth="lg">
        ...
        >
          ...
          >
            ...
            >
              <CheckoutSummary items={enrichedItems} totalPrice={totalPrice} />
              <Box sx={{ mt: 4 }}>
                <PlaceOrderButton formData={formData} disabled={hasUnavailable} />
              </Box>
            </AppCard>
          </Box>
        </Stack>
